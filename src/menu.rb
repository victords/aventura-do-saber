...
class MenuButton < Button
	def initialize a, b, c, d, e, back = false, f = 0, g = 0, h = true, i = 0, j = 0, k = 0, l = 0, m = 0, &n
  ...
  def click
		super
		G.play_sound @back ? :backButtonClick : :buttonClick
	end
end

class MenuToggleButton < ToggleButton
  def initialize x, y, text, checked, &action
    super x, y, G.med_font, text, :ui_check, checked, 0, 0, false, 60, 10, nil, nil, nil, &action
  end

  def click
    super
    G.play_sound :toggleButtonClick
  end
end
...
class MainMenu < Menu
	def initialize screen = 0
		...
		@back_button = MenuButton.new(600, 555, G.font, "Voltar", :ui_btn1, true) { go_to_screen 0 }
    ...
    (0..3).each do |i|
      ...
      @w_answers_labels << b
		...
		Dir["#{Res.prefix}save/*"].sort[1..10].each_with_index do |g, i|
			...
			score_choose_screen_components <<
				MenuButton.new(100 + (i % 2) * 300, 140 + (i / 2) * 40, G.med_font, name.capitalize, :ui_btn2){
					...
					scenes.each_with_index { |s, j| @scenes_labels[j].text = s }
					c_answers.each_with_index { |s, j| @c_answers_labels[j].text = s.empty? ? 0 : s[-1] }
					w_answers.each_with_index { |s, j| @w_answers_labels[j].text = s }
					series = []
		...
		@chk1 = MenuToggleButton.new(40, 110, "Tela cheia", G.full_screen) { |c|
			G.set_option 0, c
		...
		@chk2 = MenuToggleButton.new(40, 170, "Mostrar dicas", G.hints) { |c| G.set_option 1, c }
		@chk3 = MenuToggleButton.new(40, 230, "Tocar sons", G.sounds) { |c| G.set_option 2, c }
		@chk4 = MenuToggleButton.new(40, 290, "Tocar músicas", G.music) { |c| G.set_option 3, c }
		...
		@screens = [
			...
			MenuScreen.new([
				...
				MenuButton.new(325, 376, G.font, "Voltar", :ui_btn1, true) { @name_input.focus; go_to_screen 1 },
				@same_name
			]),
			MenuScreen.new([
				MenuPanel.new(-660, 0, 0, 0, :ui_menuComponent1),
				...
				MenuButton.new(600, 555, G.font, "Voltar", :ui_btn1, true) { @name_input.focus; go_to_screen 1 },
				MenuButton.new(122, 97, nil, nil, nil, false, 0, 0, 0, 0, 0, 212, 420) {
					@char = :marcus
					@char_name.text = "Marcus"
					@char_description.text = "Um simpático garoto de 10 anos."
					@char_selection.set_pos 112, 87
				},
				MenuButton.new(466, 97, nil, nil, nil, false, 0, 0, 0, 0, 0, 212, 420) {
					@char = :milena
			...
			MenuScreen.new([
				...
				MenuButton.new(600, 555, G.font, "Voltar", :ui_btn1, true) { go_to_screen 3 },
				MenuButton.new(50, 110, nil, nil, nil, false, 0, 0, 0, 0, 0, 320, 180) { @game_type = 0; @game_selection.set_pos 38, 98 },
				MenuButton.new(430, 110, nil, nil, nil, false, 0, 0, 0, 0, 0, 320, 180) { @game_type = 1; @game_selection.set_pos 418, 98 },
				MenuButton.new(50, 330, nil, nil, nil, false, 0, 0, 0, 0, 0, 320, 180) { @game_type = 2; @game_selection.set_pos 38, 318 },
				MenuButton.new(430, 330, nil, nil, nil, false, 0, 0, 0, 0, 0, 320, 180) { @game_type = 3; @game_selection.set_pos 418, 318 },
	...
	def initialize game_type
		...
		@screens = [
			MenuScreen.new([
				...
				MenuButton.new(325, 230, G.font, "Continuar", :ui_btn1, true) { G.resume_game },
			...
			MenuScreen.new([
				...
				MenuButton.new(405, 360, G.font, "Cancelar", :ui_btn1, true) { go_to_screen 0 },
